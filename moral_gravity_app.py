#!/usr/bin/env python3
"""
Moral Gravity Wells Analysis - Streamlit Interface
A user-friendly GUI for the Moral Gravity Wells framework
"""
...
# Force reload moral_gravity_elliptical to ensure we get the latest changes
if 'moral_gravity_elliptical' in sys.modules:
    importlib.reload(sys.modules['moral_gravity_elliptical'])

# Import our existing modules
from moral_gravity_elliptical import MoralGravityWellsElliptical
from framework_manager import FrameworkManager
from generate_prompt import PromptGenerator

# Configure page
st.set_page_config(
    page_title="Moral Gravity Wells Analysis",
    page_icon="🎯",
    layout="wide",
...
def main_analysis_interface():
    ...
    st.markdown('<div class="main-header">🎯 Moral Gravity Wells Analysis</div>', unsafe_allow_html=True)
        ...
        if 'generated_prompt' in st.session_state:
            ...
            with col_btn1:
                st.download_button(
                    ...
                    data=prompt,
                    file_name=f"moral_gravity_prompt_{timestamp}.txt",
                    mime="text/plain"
                )
...
def display_analysis_results():
    ...
    if Path(results['png_path']).exists():
        st.image(results['png_path'], caption="Moral Gravity Wells Visualization", use_container_width=True)
    ...
    with col1:
        ...
        if 'calculated_metrics' in data:
            ...
            st.metric("Moral Elevation", f"{metrics.get('moral_elevation', 0):.3f}")
            st.metric("Moral Polarity", f"{metrics.get('moral_polarity', 0):.3f}")
...
def framework_creation_interface():
    ...
    with st.form("framework_creator"):
        ...
        num_dipoles = st.number_input(
            ...
            max_value=8,
            value=5,
            help="How many moral dipoles will your framework include?"
        )
        ...
        if submitted and framework_name and len(dipoles) == num_dipoles:
            try:
                ...
                framework_config = {
                    ...
                    "ellipse": {
                        "description": "Elliptical coordinate system parameters",
                        "semi_major_axis": 1.0,
                        "semi_minor_axis": 0.7,
...
def display_comparative_results():
    ...
    # Display visualization
    if Path(results['png_path']).exists():
        st.image(results['png_path'], caption="Moral Distance Analysis", use_container_width=True)
    ...
    with col1:
        ...
        st.markdown("**🎯 Purpose:** Shows moral distance between two narratives")
        st.markdown("**📏 Key Metric:** Elliptical distance between moral centers")
        st.markdown("**🔗 Visual Elements:**")
        st.markdown("- Two moral centers positioned within the ellipse")
        st.markdown("- Dotted line connecting the moral centers") 
        st.markdown("- Narrative labels positioned over each center")
    
    with col2:
        st.markdown("**🎨 Visualization Features:**")
        st.markdown("- Moral wells positioned on ellipse boundary")
...
def create_comparison_chart(data1, data2, name1, name2, selected_framework_name: str):
    ...
    if scores1 and scores2:
        ...
        if common_wells:
            # Create elliptical comparative visualization
            try:
                analyzer = MoralGravityWellsElliptical()
                ...
                if Path(output_path).exists():
                    st.image(output_path, caption="Elliptical Comparative Analysis", use_container_width=True)
                    ...
                    st.metric("Narrative Distance", f"{distance:.3f}", 
                             help="Elliptical distance between the two moral centers")
                    
                    # Add the improved score differences table below the elliptical visualization
                    ...
                    column_config = {
                        "Well": st.column_config.TextColumn(
                            "Moral Well",
                            width="medium",
                            help="The moral foundation being compared"
                        ),
                ...
                st.error(f"Error creating elliptical comparison: {e}")
                # Fallback to bar chart if elliptical fails
                create_fallback_bar_chart(data1, data2, name1, name2, scores1, scores2, common_wells)
        else:
            st.error("No common moral wells found between the analyses.")
    else:
...
def create_fallback_bar_chart(data1, data2, name1, name2, scores1, scores2, common_wells):
    """Create fallback bar chart if elliptical visualization fails"""
    # Extract actual narrative titles from metadata
    title1 = data1.get('metadata', {}).get('title', name1)
    title2 = data2.get('metadata', {}).get('title', name2)
    ...
    fig.update_layout(
        title="Moral Wells Scores Comparison (Fallback)",
        xaxis_title="Moral Wells",
        yaxis_title="Scores",
        barmode='group',
        height=500
    )
    ...
    with col1:
        # Moral Elevation
        if ('calculated_metrics' in data1 and 'moral_elevation' in data1['calculated_metrics'] and
            'calculated_metrics' in data2 and 'moral_elevation' in data2['calculated_metrics']):
            val1 = data1['calculated_metrics']['moral_elevation']
            val2 = data2['calculated_metrics']['moral_elevation']
            delta = val2 - val1
            st.metric("Moral Elevation", f"{val2:.3f}", delta=f"{delta:+.3f}")
    
    with col2:
        # Moral Polarity
        if ('calculated_metrics' in data1 and 'moral_polarity' in data1['calculated_metrics'] and
            'calculated_metrics' in data2 and 'moral_polarity' in data2['calculated_metrics']):
            val1 = data1['calculated_metrics']['moral_polarity']
            val2 = data2['calculated_metrics']['moral_polarity']
            delta = val2 - val1
            st.metric("Moral Polarity", f"{val2:.3f}", delta=f"{delta:+.3f}")
...
def main():
    ...
    st.markdown(
        ...
        'Moral Gravity Wells Framework v2.0 | Built with Streamlit'
        '</div>',
        unsafe_allow_html=True
    )
