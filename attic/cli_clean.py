...
from discernus.core.validation import ExperimentValidator
from discernus.core.manifest_loader import ManifestLoader

# Import provenance and documentation components
from discernus.core.provenance_consolidator import consolidate_provenance_data
...
def validate(ctx, experiment_path: str, dry_run: bool):
    ...
    try:
        ...
        validator = ExperimentValidator(exp_path)
        
        # Run validation
        validation_result = validator.validate_experiment()
        
        if validation_result.is_valid:
            rich_console.print_success("✅ Experiment validation passed!")
            rich_console.print_info(f"   📁 Experiment: {exp_path}")
            rich_console.print_info(f"   📊 Documents: {validation_result.corpus_document_count}")
            rich_console.print_info(f"   🎯 Framework: {validation_result.framework_name}")
            exit_success()
        else:
            rich_console.print_error("❌ Experiment validation failed:")
            for error in validation_result.errors:
                rich_console.print_error(f"   • {error}")
            exit_validation_failed("Experiment validation failed")
...
def consolidate_provenance(run_directory: str, output: Optional[str]):
    ...
    try:
        provenance_data = consolidate_provenance_data(run_path, output_path)
...
def validate_score(experiment_path: str, document_name: str, score_name: str, 
    ...
    # Validate model against registry before proceeding
    models_to_validate = [("validation", model)]
    _validate_models(models_to_validate)
