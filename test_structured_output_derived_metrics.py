#!/usr/bin/env python3
"""
Unit test for structured output derived metrics generation.
Tests that we get clean code directly from the LLM without parsing.
"""
...
def test_structured_output_derived_metrics():
    """Test that derived metrics agent uses structured output to get clean code."""
...
"""
        ...
        experiment_spec = {
            ...
            "description": "Test experiment for structured output",
        ...
        # Create analysis data file (required by _load_data_structure)
        analysis_data = [
            {"sentiment": 0.8, "confidence": 0.9, "document_id": "doc1"},
            {"sentiment": -0.3, "confidence": 0.7, "document_id": "doc2"}
        ]
        (workspace_path / "analysis_data.json").write_text(json.dumps(analysis_data))
        
        print("🧪 Testing structured output derived metrics generation...")
        
        # Debug: Check what files we created
        print(f"🔍 Workspace contents: {list(workspace_path.iterdir())}")
        print(f"🔍 Framework content exists: {(workspace_path / 'framework_content.md').exists()}")
        print(f"🔍 Experiment spec exists: {(workspace_path / 'experiment_spec.json').exists()}")
        print(f"🔍 Analysis data exists: {(workspace_path / 'analysis_data.json').exists()}")
        ...
        try:
            ...
            # Test 1: Check that code starts with imports (no markdown wrappers)
            if generated_code.strip().startswith('```'):
                raise Exception("Code still has markdown wrappers - structured output not working")
            
            if not generated_code.strip().startswith('import '):
                raise Exception("Code doesn't start with imports - unexpected format")
            
            print("✅ Code starts with imports (no markdown wrappers)")
            
            # Test 2: Check for basic Python syntax
            try:
            ...
            try:
                ...
                # Test 5: Check if functions are callable
            ...
            print("🎉 All tests passed! Structured output working correctly.")
            return True
...
if __name__ == "__main__":
    success = test_structured_output_derived_metrics()
