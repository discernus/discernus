...
class LiteLLMClient:
    ...
    def _parse_response(self, content: str, text_input: str = None, framework: str = None) -> Dict[str, Any]:
        ...
        # Step 2: Quality assurance validation (preserved from DirectAPIClient)
        try:
            from src.utils.llm_quality_assurance import LLMQualityAssuranceSystem
            qa_system = LLMQualityAssuranceSystem()
            
            # Run quality assurance with both text and framework context
            qa_result = qa_system.validate_llm_analysis(
                text_input or self._current_text or "",
                framework or self._current_framework or "",
                raw_response,
                parsed_scores
            )
            
            # Merge QA results into the response
            result = {
                'scores': parsed_scores,
                'raw_response': content,
                'parsed': True,
                'qa_validation': qa_result,
                'analysis_quality': qa_result.get('overall_score', 0.0),
            ...
            print("⚠️ LLMQualityAssuranceSystem not available - skipping QA validation")
            result = {
                'scores': parsed_scores,
                'raw_response': content,
                'parsed': True,
                'qa_validation': {'passed': True, 'note': 'QA system not available'},
                'analysis_quality': 1.0,
                'qa_passed': True
            }
        except Exception as qa_error:
            print(f"⚠️ Quality assurance failed: {qa_error}")
            result = {
                'scores': parsed_scores,
                'raw_response': content,
                'parsed': True,
                'qa_validation': {'passed': False, 'error': str(qa_error)},
