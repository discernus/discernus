...
class NarrativeGravityWellsElliptical:
    ...
    def calculate_narrative_position(self, well_scores: Dict[str, float]) -> Tuple[float, float]:
        ...
        for well_name, score in well_scores.items():
            if well_name in self.well_definitions:
                ...
                force = score * abs(narrative_weight)
    ...
    def calculate_elliptical_metrics(self, narrative_x: float, narrative_y: float, 
        ...
        # Narrative Elevation Score: Y-coordinate normalized by semi-major axis (vertical)
        narrative_elevation = narrative_y / self.ellipse_a
        
        # Enhanced Narrative Polarity: Distance from center normalized
        narrative_polarity = np.sqrt(narrative_x**2 + narrative_y**2) / max(self.ellipse_a, self.ellipse_b)
        ...
        # Directional Purity Score (DPS): Alignment with vertical narrative axis
        dps = self.calculate_directional_purity_score(well_scores)
        
        return {
            'narrative_elevation': narrative_elevation,
    ...
    def calculate_directional_purity_score(self, well_scores: Dict[str, float]) -> float:
        ...
        for well_name, score in well_scores.items():
            if well_name in self.well_definitions:
                ...
                numerator += narrative_weight * score * np.sign(well_y)
                denominator += abs(narrative_weight) * score
    ...
    def add_metrics_display(self, metrics: Dict[str, float]) -> None:
        ...
        metrics_text = (f"Narrative Elevation: {metrics['narrative_elevation']:.3f}\n"
                       f"Narrative Polarity: {metrics['narrative_polarity']:.3f}\n"
    ...
    def create_comparative_visualization(self, analyses: List[Dict], output_path: str = None) -> str:
        ...
        self.fig.text(0.5, 0.95, "Narrative Distance Analysis",
                     fontsize=self.style_config['font_sizes']['title'],
                     fontweight='bold',
                     horizontalalignment='center',
                     color=self.style_config['colors']['text_primary'])
        ...
        for i, data in enumerate(normalized_analyses):
            ...
            # Center the label over the narrative center
        ...
        # Add dotted line between narrative centers (if there are exactly 2)
        if len(positions) == 2:
            pos1, pos2 = positions
    ...
    def add_comparative_metrics(self, positions: List[Tuple[float, float]], analyses: List[Dict]) -> None:
        ...
        for i, analysis in enumerate(analyses):
            ...
            metrics_text += f"\n{title_short}:\n"
            metrics_text += f"  Elevation: {metrics['narrative_elevation']:.3f}\n"
...
def main():
    """Main function for generating narrative gravity visualizations."""
    parser = argparse.ArgumentParser(
        description='Generate narrative gravity visualizations from JSON analysis files'
    )
