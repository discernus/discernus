#!/usr/bin/env python3
"""
Create Obama Elliptical + Enhanced Bar Chart Dashboard V7
Centered layout with Statistical Narrative focused on variance analysis
"""
...
def generate_statistical_narrative(mean_scores, well_stats, narrative_stats):
    """Generate statistical narrative focused on variance analysis and data reliability"""
    
    print("🤖 Generating statistical narrative...")
    ...
    variance_summary = f"""Variance Analysis Summary:
...
- Narrative center variance: X = {narrative_stats['x']['std']:.4f}, Y = {narrative_stats['y']['std']:.4f}
- Integrative wells avg variance: {np.mean(int_variances):.4f}
- Disintegrative wells avg variance: {np.mean(dis_variances):.4f}"""

    prompt = f"""You are analyzing the statistical reliability of a multi-run narrative gravity analysis. Focus exclusively on variance analysis to assess data reliability.

{variance_summary}

Provide a statistical narrative that addresses:
1. Overall assessment of data reliability based on variance patterns
2. Which categories (integrative vs disintegrative) show more consistency
3. Statistical significance of the variance patterns observed
4. Whether the 5-run sample size provides reliable data for conclusions
5. Any variance outliers or patterns that affect reliability

Keep response under 150 words and focus purely on statistical assessment of reliability."""
    ...
    try:
        ...
        if result and 'error' not in result:
            # Extract the analysis text
            if isinstance(result, dict):
                return result.get('analysis', result.get('raw_response', 'Statistical narrative generation failed.'))
            else:
                return str(result)
        else:
            return "Unable to generate statistical narrative."
            
    except Exception as e:
        print(f"Error generating statistical narrative: {e}")
        return "Error generating statistical narrative."
...
def create_dashboard_visualization():
    """Create the centered dashboard with Statistical Narrative focused on variance analysis"""
    ...
    # Create figure with centered layout
    fig = plt.figure(figsize=(20, 12))
    
    # Use GridSpec with centered positioning - add left padding
    from matplotlib.gridspec import GridSpec
    gs = GridSpec(5, 6, figure=fig, 
                  height_ratios=[0.5, 2, 2, 0.3, 1.2], 
                  width_ratios=[0.7, 1.8, 2.2, 0.7, 0.7, 0.2],  # Added left padding and adjusted
                  hspace=0.3, wspace=0.1)
    
    # Create the elliptical subplot (left) - now starts at column 1
    ...
    # Create the enhanced bar chart (right) - now at column 2
    ...
    print("   🤖 Generating statistical narrative...")
    statistical_narrative = generate_statistical_narrative(mean_scores, well_stats, narrative_stats)
    
    # Add bottom panels with centered positioning
    print("   📝 Adding summary and statistical narrative panels...")
    
    # Left bottom panel - Composite Summary (aligned with ellipse)
    ...
    # Right bottom panel - Statistical Narrative (spans remaining columns)
    ax4 = fig.add_subplot(gs[4, 2:5])  # Span columns 2-4 for statistical narrative
    ax4.axis('off')
    
    # Better text formatting
    ax4.text(0.02, 0.95, "STATISTICAL NARRATIVE - VARIANCE ANALYSIS", 
             fontsize=14, fontweight='bold', color='darkred',
             transform=ax4.transAxes, va='top')
    
    # Wrap text with appropriate width for the expanded panel
    wrapped_narrative = textwrap.fill(statistical_narrative, width=85)
    ax4.text(0.02, 0.75, wrapped_narrative, 
...
def main():
    """Generate the centered dashboard with Statistical Narrative"""
    print("🎨 Creating Obama Multi-Run Analysis Dashboard V7...")
    ...
    filename = f"test_results/obama_dashboard_v7_{timestamp}.png"
    
    fig.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    
    print(f"✅ Dashboard V7 saved: {filename}")
    print(f"📊 Features: Centered layout with Statistical Narrative")
    print(f"🎯 Improvements: Centered positioning, variance-focused analysis")
