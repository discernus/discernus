#!/usr/bin/env python3
"""
Create Obama Elliptical + Enhanced Bar Chart Dashboard V6
Fine-tuned layout with better spacing and alignment
"""
...
def create_dashboard_visualization():
    """Create the fine-tuned dashboard with better spacing and alignment"""
    ...
    # Create figure with fine-tuned proportions
    fig = plt.figure(figsize=(20, 12))
    
    # Use GridSpec with adjusted column widths and spacing
    from matplotlib.gridspec import GridSpec
    gs = GridSpec(5, 4, figure=fig, 
                  height_ratios=[0.5, 2, 2, 0.3, 1.2], 
                  width_ratios=[1.8, 2.2, 0.1, 0.1],  # Adjusted widths for better spacing
                  hspace=0.3, wspace=0.1)  # Reduced wspace to bring charts closer
    
    # Create the elliptical subplot (left) - spans first column
    ...
    # Create the enhanced bar chart (right) - spans second column, stretched left
    ...
    # Add bottom panels with improved sizing and alignment
    print("   📝 Adding summary and takeaways panels...")
    
    # Left bottom panel - Composite Summary (aligned with ellipse and expanded 10%)
    ...
    # Wrap text with expanded width (10% more characters)
    wrapped_summary = textwrap.fill(composite_summary, width=66)  # Increased from 60
    ...
    # Right bottom panel - Takeaways (widened to span both remaining columns)
    ax4 = fig.add_subplot(gs[4, 1:])  # Span columns 1-3 for wider takeaways
    ax4.axis('off')
    ...
    # Wrap text with more width for the expanded panel
    wrapped_takeaways = textwrap.fill(takeaways, width=85)  # Increased from 60
...
def main():
    """Generate the fine-tuned dashboard visualization"""
    print("🎨 Creating Obama Multi-Run Analysis Dashboard V6...")
    ...
    filename = f"test_results/obama_dashboard_v6_{timestamp}.png"
    
    fig.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    
    print(f"✅ Dashboard V6 saved: {filename}")
    print(f"📊 Features: Fine-tuned spacing and alignment")
    print(f"🎯 Improvements: Stretched right chart, widened takeaways, aligned composite summary")
