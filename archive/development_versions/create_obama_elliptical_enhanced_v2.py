#!/usr/bin/env python3
"""
Create Obama Elliptical + Enhanced Bar Chart Visualization V2
Left: Elliptical visualization with mean scores
Right: Enhanced integrative vs disintegrative bar chart + Narrative Center coordinates
"""
...
def create_combined_visualization():
    """Create the combined two-panel visualization with narrative center bars"""
    ...
    print("   📊 Generating enhanced bar chart with narrative center...")
    ...
    # Add narrative center coordinates
    narrative_means = [narrative_stats['x']['mean'], narrative_stats['y']['mean']]
    narrative_stds = [narrative_stats['x']['std'], narrative_stats['y']['std']]
    narrative_labels = ['Narrative X', 'Narrative Y']
    
    # Create bars
    x_int = np.arange(len(integrative_wells))
    x_dis = np.arange(len(disintegrative_wells)) + len(integrative_wells) + 0.5
    x_narrative = np.arange(len(narrative_labels)) + len(integrative_wells) + len(disintegrative_wells) + 1.0
    ...
    # Plot narrative center coordinates (using a different color)
    bars_narrative = ax2.bar(x_narrative, narrative_means, yerr=narrative_stds, capsize=5,
                            color='#4169E1', alpha=0.7, label='Narrative Center (COM)',
                            edgecolor='black', linewidth=1)
    ...
    # Add value labels for narrative center coordinates
    for i, (bar, mean, std) in enumerate(zip(bars_narrative, narrative_means, narrative_stds)):
        height = bar.get_height()
    ...
    # Customize the chart
    all_wells = integrative_wells + disintegrative_wells + narrative_labels
    all_positions = list(x_int) + list(x_dis) + list(x_narrative)
    
    ax2.set_xticks(all_positions)
    ax2.set_xticklabels(all_wells, rotation=45, ha='right', fontsize=11)
    ax2.set_ylabel('Score / Coordinate Value', fontsize=12, fontweight='bold')
    ax2.set_title('Wells + Narrative Center Coordinates\nwith Confidence Intervals (±1 Standard Deviation)', 
                  fontsize=14, fontweight='bold', pad=20)
    ax2.legend(loc='upper left', fontsize=11)
    ax2.grid(True, alpha=0.3)
    
    # Adjust y-limits to accommodate narrative center coordinates (which can be negative)
    y_min = min(min(dis_means) - max(dis_stds), min(narrative_means) - max(narrative_stds)) - 0.1
    y_max = max(max(int_means) + max(int_stds), max(narrative_means) + max(narrative_stds)) + 0.1
    ax2.set_ylim(y_min, y_max)
    
    # Add dividing lines between sections
    ax2.axvline(x=len(integrative_wells) + 0.25, color='gray', linestyle='--', alpha=0.5)
    ax2.axvline(x=len(integrative_wells) + len(disintegrative_wells) + 0.75, color='gray', linestyle='--', alpha=0.5)
...
def main():
    ...
    print("🎨 Creating Obama Multi-Run Elliptical + Enhanced Bar Chart V2...")
    ...
    filename = f"test_results/obama_elliptical_enhanced_v2_{timestamp}.png"
    
    fig.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    
    print(f"✅ Enhanced V2 visualization saved: {filename}")
    print(f"📊 Left Panel: Elliptical map with mean scores")
    print(f"📊 Right Panel: Enhanced bar chart with wells + narrative center coordinates")
    print(f"🎯 New Feature: Narrative Center (COM) X,Y coordinates with confidence intervals")
