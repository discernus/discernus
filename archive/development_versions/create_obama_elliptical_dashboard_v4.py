#!/usr/bin/env python3
"""
Create Obama Elliptical + Enhanced Bar Chart Dashboard V4
Extended canvas with LLM-generated composite summary and takeaways
"""
...
def generate_composite_summary(all_analyses):
    """Generate a composite summary from all individual analyses"""
    ...
    prompt = f"""Based on these 5 separate analyses of Obama's inaugural speech, create a composite summary that synthesizes the key findings. Focus on the most consistent themes and insights that emerge across all analyses.

Individual Analyses:
{combined_text}

Please provide a concise composite summary (3-4 sentences) that captures the essence of what all analyses agree upon regarding Obama's narrative gravity profile."""
...
def generate_takeaways(mean_scores, well_stats, narrative_stats):
    """Generate takeaways and discussion of results"""
...
- Most variable well: {max(well_stats.keys(), key=lambda w: well_stats[w]['std'])} (±{max(well_stats.values(), key=lambda s: s['std'])['std']:.3f})"""

    prompt = f"""Based on this multi-run analysis of Obama's inaugural speech using the civic virtue framework, provide key takeaways and discuss what these results mean.

{stats_summary}

Please provide:
1. What the exceptional consistency across runs tells us about the model and the text
2. What Obama's civic virtue profile reveals about his rhetorical approach
3. The significance of the narrative center position
4. Broader implications for narrative gravity analysis

Provide 4-5 key takeaways in a discussion format (keep under 200 words)."""
...
def create_dashboard_visualization():
    """Create the complete dashboard with charts and LLM-generated insights"""
    ...
    # Create the extended figure (20x14 instead of 20x10)
    fig = plt.figure(figsize=(20, 14))
    
    # Create the elliptical subplot (left) - moved down
    print("   📊 Generating elliptical visualization...")
    ax1 = plt.subplot2grid((7, 2), (1, 0), rowspan=3)  # Position moved down by 1 row
    ...
    # Create the enhanced bar chart (right) - moved down
    print("   📊 Generating enhanced bar chart...")
    ax2 = plt.subplot2grid((7, 2), (1, 1), rowspan=3)  # Position moved down by 1 row
    ...
    # Add bottom panels with LLM-generated content
    ...
    wrapped_summary = textwrap.fill(composite_summary, width=80)
    ax3.text(0.02, 0.98, "COMPOSITE SUMMARY", 
             fontsize=12, fontweight='bold', 
             transform=ax3.transAxes, va='top')
    ax3.text(0.02, 0.85, wrapped_summary, 
    ...
    # Wrap text for better display
    wrapped_takeaways = textwrap.fill(takeaways, width=80)
    ax4.text(0.02, 0.98, "KEY TAKEAWAYS & DISCUSSION", 
             fontsize=12, fontweight='bold', 
             transform=ax4.transAxes, va='top')
    ax4.text(0.02, 0.85, wrapped_takeaways, 
...
def main():
    """Generate the complete dashboard visualization"""
    print("🎨 Creating Obama Multi-Run Analysis Dashboard V4...")
    ...
    filename = f"test_results/obama_dashboard_v4_{timestamp}.png"
    
    fig.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    
    print(f"✅ Dashboard V4 saved: {filename}")
    print(f"📊 Features: Extended canvas with LLM-generated insights")
    print(f"🎯 Layout: Charts (top) + Composite Summary + Takeaways (bottom)")
