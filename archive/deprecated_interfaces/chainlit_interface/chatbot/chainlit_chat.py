...
try:
    ...
    try:
        ...
        from src.narrative_gravity.chatbot.narrative_gravity_bot import NarrativeGravityBot
        from src.narrative_gravity.framework_manager import FrameworkManager
        from src.narrative_gravity.utils.environment_setup import setup_environment, check_database_connection
        from src.narrative_gravity.models.database import SessionLocal
    except ImportError as e2:
        print(f"Alternative import also failed: {e2}")
        # Create minimal fallback
        class NarrativeGravityBot:
            def process_message(self, msg):
                return f"**Analysis Request Received**: {msg[:100]}...\n\n*Note: Full analysis engine not available. This is the LLM Validation Workbench interface.*"
        
        class FrameworkManager:
            def list_frameworks(self):
                return [
                    {"name": "fukuyama_identity", "version": "v2025.01.07", "description": "Fukuyama Identity Framework for democratic sustainability analysis"},
                    {"name": "civic_virtue", "version": "v1.0.0", "description": "Civic virtue and democratic values framework"},
                    {"name": "political_spectrum", "version": "v1.0.0", "description": "Left-right political spectrum analysis"},
                ]
...
- **Target Framework**: Which framework to validate (current: {})
...
""".format(framework_manager.get_active_framework() if framework_manager else "None")).send()
