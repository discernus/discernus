...
def main():
    ...
    results = analyzer.analyze_reliability(structured_results)
    
    # Save results
    output_dir = Path('experiment_reports/analysis')
    output_dir.mkdir(parents=True, exist_ok=True)
    
    output_file = output_dir / f'reliability_analysis_results_{pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")}.json'
    
    with open(output_file, 'w') as f:
        json.dump(results, f, indent=2, default=str)
    
    logger.info(f"✅ Reliability analysis results saved to: {output_file}")
    
    # Print summary
    if 'reliability_summary' in results:
        summary = results['reliability_summary']
        print(f"\n🎯 RELIABILITY ANALYSIS SUMMARY")
        print(f"Overall Reliability: {summary.get('overall_reliability', 'unknown')}")
