...
class LocalArtifactStorage:
    ...
    def put_artifact(self, content: bytes, metadata: Optional[dict] = None, human_context: Optional[dict] = None) -> str:
        """
        ...
        Args:
            ...
            human_context: Optional human-readable context for enhanced naming
        ...
        try:
            ...
            human_filename = self._generate_human_filename(metadata, short_hash, human_context)
            ...
            subdirectory = self._get_artifact_subdirectory(artifact_type)
            
            # Create subdirectory if it doesn't exist
            subdir_path = self.artifacts_dir / subdirectory
            subdir_path.mkdir(parents=True, exist_ok=True)
            
            # Store the artifact with human-readable filename in appropriate subdirectory
            artifact_path = subdir_path / human_filename
    ...
    def _generate_human_filename(self, metadata: Optional[dict], short_hash: str, human_context: Optional[dict] = None) -> str:
        """
        ...
        if artifact_type == "analysis_json_v6":
            ...
            if human_context and human_context.get("source_document_name"):
                source_name = human_context["source_document_name"]
                clean_name = self._clean_filename(source_name)
                return f"composite_analysis_{clean_name}_{short_hash}.json"
            ...
            if human_context and human_context.get("source_document_name"):
                source_name = human_context["source_document_name"]
                clean_name = self._clean_filename(source_name)
                return f"evidence_extraction_{clean_name}_{short_hash}.json"
            ...
            if human_context and human_context.get("source_document_name"):
                source_name = human_context["source_document_name"]
                clean_name = self._clean_filename(source_name)
                return f"score_extraction_{clean_name}_{short_hash}.json"
            ...
            if human_context and human_context.get("source_document_name"):
                ...
                clean_name = self._clean_filename(source_name)
                return f"marked_up_document_{clean_name}_{short_hash}.md"
        ...
        elif artifact_type == "experiment_spec":
            # Enhanced naming with human context
            if human_context and human_context.get("experiment_name"):
                exp_name = human_context["experiment_name"]
                clean_name = self._clean_filename(exp_name)
                return f"experiment_spec_{clean_name}_{short_hash}.md"
            return f"experiment_spec_{short_hash}.md"
        elif artifact_type == "evidence_appendix":
            return f"evidence_appendix_{short_hash}.md"
        else:
    ...
    def get_artifact(self, hash_id: str, quiet: bool = False) -> bytes:
        ...
        try:
            ...
            # Try to find the artifact in subdirectories first
            artifact_path = None
            for subdir in self.artifacts_dir.iterdir():
                if subdir.is_dir():
                    potential_path = subdir / human_filename
                    if potential_path.exists():
                        artifact_path = potential_path
                        break
            
            # Fallback to root directory for legacy artifacts
    ...
    def find_artifacts_by_metadata(self, **filters) -> List[str]:
        ...
        """
        # Defensive programming: Check if registry is properly initialized
        if self.registry is None:
            print(f"⚠️ WARNING: Registry is None in find_artifacts_by_metadata with filters: {filters}")
            return []
    ...
    def _get_artifact_subdirectory(self, artifact_type: str) -> str:
        """
        Determine the appropriate subdirectory for an artifact type.
        
        Args:
            artifact_type: The type of artifact
            
        Returns:
            Subdirectory name for the artifact
        """
        # Define directory mapping based on artifact type
        if artifact_type in ["experiment_spec", "framework", "corpus_manifest"]:
            return "experiment"
        elif artifact_type in ["corpus_document"]:
            return "source_documents"
        elif artifact_type in ["composite_analysis", "evidence_extraction", "score_extraction", "marked_up_document"]:
            return "analysis"
        elif artifact_type in ["statistical_analysis", "statistical_verification", "derived_metrics"]:
            return "results"
        elif artifact_type in ["validation_report", "synthesis_report", "final_synthesis_report", "stage1_synthesis_report"]:
            return "results"
        elif artifact_type in ["run_context"]:
            return "provenance"
        else:
            return "misc"
