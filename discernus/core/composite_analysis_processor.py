"""
Score Extraction Statistical Processor

Processes clean score_extraction artifacts generated by the Analysis Agent's enhanced
score extraction step. This approach leverages LLM intelligence to clean and validate
JSON data, then uses deterministic processing for statistical analysis.

The score extraction contains clean, validated data:
- Dimensional scores (raw_score, salience, confidence, reliability, status)
- Derived metrics (framework-calculated values)
- JSON validation and self-correction by LLM
- Reasoning-enabled extraction for better accuracy
"""
...
class ScoreExtractionProcessor:
    """
    Statistical processor that works with clean score_extraction artifacts.
    
    Advantages of this approach:
    1. LLM validates and cleans its own JSON output
    2. Reasoning-enabled extraction for better accuracy
    ...
    def process_score_extractions(self, artifact_paths: List[Path]) -> Dict[str, Any]:
        """
        Process score_extraction artifacts and return comprehensive statistics.
        ...
        """
        # Step 1: Ingest and parse all score extractions
        raw_data = self._ingest_score_extractions(artifact_paths)
    ...
    def _ingest_score_extractions(self, artifact_paths: List[Path]) -> List[Dict[str, Any]]:
        """Ingest and parse score_extraction artifacts."""
        parsed_data = []
        ...
        for path in artifact_paths:
            try:
                ...
                # Extract the score extraction JSON from LLM response
                if 'score_extraction' in artifact_data:
                    score_data = artifact_data['score_extraction']
                    
                    # Clean markdown formatting if present
                    if isinstance(score_data, str):
                        if score_data.startswith('```json\n'):
                            score_data = score_data[7:]
                        elif score_data.startswith('```json'):
                            score_data = score_data[7:]
                        if score_data.endswith('\n```'):
                            score_data = score_data[:-4]
                        elif score_data.endswith('```'):
                            score_data = score_data[:-3]
                        
                        # Additional cleaning for any remaining newlines at start
                        score_data = score_data.lstrip('\n')
                        
                        try:
                            # Parse the clean JSON
                            extraction_data = json.loads(score_data)
                        except json.JSONDecodeError as e:
                            print(f"WARNING: Could not parse score extraction JSON from {path.name}: {e}")
                            # Create minimal structure for failed parsing
                            extraction_data = {
                                'dimensional_scores': {},
                                'derived_metrics': {}
                            }
    ...
    def _analyze_document_data(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'by_document': self._analyze_documents_individually(numeric_cols),
    ...
    def _analyze_documents_individually(self, numeric_cols: List[str]) -> Dict[str, Dict[str, Any]]:
        """Analyze each document individually for comparative analysis."""
        by_document = {}
        
        for _, row in self.document_data.iterrows():
            doc_id = row.get('document_id', 'unknown')
            doc_name = row.get('document_name', 'unknown')
            
            # Calculate metrics for this document only
            doc_metrics = {}
            for col in numeric_cols:
                if pd.notna(row[col]):
                    doc_metrics[col] = float(row[col])
                else:
                    doc_metrics[col] = None
            
            by_document[doc_id] = {
                'document_name': doc_name,
                'metrics': doc_metrics,
                'metric_count': len([v for v in doc_metrics.values() if v is not None]),
    ...
    def _analyze_by_dimension(self, score_cols: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for dimension in self.dimension_data['dimension'].unique():
            ...
            by_dimension[dimension] = {
                ...
                'by_document': self._analyze_dimension_by_document(dimension, dim_data, score_cols)
            }
        
        return by_dimension
    
    def _analyze_dimension_by_document(self, dimension: str, dim_data: pd.DataFrame, score_cols: List[str]) -> Dict[str, Dict[str, Any]]:
        """Analyze dimension scores by document for comparative analysis."""
        by_document = {}
        
        for doc_id in dim_data['document_id'].unique():
            doc_data = dim_data[dim_data['document_id'] == doc_id]
            doc_name = doc_data['document_name'].iloc[0] if not doc_data.empty else 'unknown'
            
            # Calculate scores for this document and dimension
            doc_scores = {}
            for col in score_cols:
                if col in doc_data.columns and pd.api.types.is_numeric_dtype(doc_data[col]):
                    if not doc_data[col].isna().all():
                        doc_scores[col] = float(doc_data[col].iloc[0])  # Should be single value per doc
                    else:
                        doc_scores[col] = None
                else:
                    doc_scores[col] = None
            ...
            by_document[doc_id] = {
                'document_name': doc_name,
                'scores': doc_scores,
                'status': doc_data['status'].iloc[0] if 'status' in doc_data.columns else 'unknown',
                'exclusion_reason': doc_data['exclusion_reason'].iloc[0] if 'exclusion_reason' in doc_data.columns else None
            }
...
def process_score_extractions(artifact_directory: Path, output_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    statistics = processor.process_score_extractions(artifact_paths)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Processed {stats['processing_metadata']['artifact_count']} score extractions")
