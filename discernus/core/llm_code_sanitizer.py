...
class LLMCodeSanitizer(cst.CSTTransformer):
    ...
    def sanitize_code(self, code: str) -> tuple[str, list[str]]:
        ...
        """
        try:
            # Step 0: Pre-emptive wrapper removal
            pre_sanitized_code = re.sub(r'^```python\n(.*?)\n```$', r'\1', code, flags=re.DOTALL)
            if pre_sanitized_code != code:
                self.transformations_applied.append("wrapper_removed")

            # Reset transformation tracking
            self.transformations_applied = []
            
            # Step 1: Pre-processing fixes for common string issues
            preprocessed_code = self._preprocess_string_literals(pre_sanitized_code)
